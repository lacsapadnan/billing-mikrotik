...
namespace Pear2\Console\CommandLine\Renderer;

use Pear2\Console\CommandLine\Renderer;
 ...
 */
class RendererDefault implements Renderer
{
    // Properties {{{
     ...
     *
     * @var Pear2\Console\CommandLine The parser
     */
    public $parser = false;
    ...
    protected function optionList()
    {
        ...
        foreach ($this->parser->options as $option) {
            ...
            $options[] = [$lines, $option->description];
            if ($col < $ln) {
                $col = $ln;
            }
        }
    ...
    protected function commandList()
    {
        ...
        foreach ($this->parser->commands as $cmdname => $command) {
            $cmdname = '  '.$cmdname;
            $commands[] = [$cmdname, $command->description, $command->aliases];
            $ln = strlen($cmdname);
            if ($col < $ln) {
                $col = $ln;
            }
