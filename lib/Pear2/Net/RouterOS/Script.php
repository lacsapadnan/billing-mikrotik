...
class Script
{
     ...
     * @return string|bool|int|float|null Depending on RouterOS type detected:
     *     - "nil" (the string "[]") or "nothing" (empty string) - NULL.
     *     - "num" - int or double for large values.
     *     - "bool" - a boolean.
     *     - Unrecognized type - casted to a string, unmodified.
    ...
    public static function parseValueToSimple($value)
    {
        ...
        if (in_array($value, ['', '[]'], true)) {
            return null;
        } elseif (in_array($value, ['true', 'false', 'yes', 'no'], true)) {
            return $value === 'true' || $value === 'yes';
        } elseif ($value === (string) ($num = (int) $value)
            || $value === (string) ($num = (float) $value)
        ) {
            return $num;
        }
    ...
    public static function parseValueToDateInterval($value)
    {
        ...
        if ('' !== $value && preg_match(
            ...
            if (! empty($time[6])) {
                $subsecondTime += ((float) $time[6]) / 1000;
            }
            if (! empty($time[7])) {
                $subsecondTime += ((float) $time[7]) / 1000000;
            }
            if (! empty($time[8])) {
                $subsecondTime += ((float) $time[8]) / 1000000000;
            }
    ...
    {
        if ($full) {
            return self::_escapeCharacters([$value]);
        }

        return preg_replace_callback(
            '/[^\\_A-Za-z0-9\\x80-\\xFF]+/S',
            [__CLASS__, '_escapeCharacters'],
            $value
        );
    }
