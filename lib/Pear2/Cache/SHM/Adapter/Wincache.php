...
class Wincache extends SHM
{
     ...
     * @param  float  $timeout Ignored with WinCache. Script will always block if
     ...
     */
    public function getIterator($filter = null, $keysOnly = false): ArrayObject
    {
        $info = wincache_ucache_info();
        $result = [];
        foreach ($info['ucache_entries'] as $entry) {
